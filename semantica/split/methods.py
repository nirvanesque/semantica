...
def split_semantic_transformer(
    ...
    **kwargs
    ...
    try:
        ...
        logger.warning(f"Error in semantic transformer splitting: {e}, falling back to sentence splitting")
        return split_by_sentences(text, chunk_size=chunk_size, **kwargs)
...
def split_llm(
    ...
    **kwargs
    ...
    if not SEMANTIC_EXTRACT_AVAILABLE:
        logger.warning("semantic_extract not available, falling back to recursive splitting")
    ...
    try:
        ...
        for rough_chunk in rough_chunks:
            ...
            try:
                split_indices = [int(x.strip()) for x in response.split(",") if x.strip().isdigit()]
        ...
        logger.warning(f"Error in LLM splitting: {e}, falling back to recursive splitting")
        return split_recursive(text, chunk_size=chunk_size, **kwargs)
...
def split_huggingface(
    ...
    **kwargs
    ...
    try:
        ...
        logger.warning(f"Error in HuggingFace splitting: {e}, falling back to token splitting")
        return split_by_tokens(text, chunk_size=chunk_size, **kwargs)
...
def split_entity_aware(
    ...
    **kwargs
    ...
    try:
                ...
                if preserve_entities and has_entity_boundary:
                    ...
                    chunks.append(Chunk(
                        ...
                        metadata={
                            "method": "entity_aware",
                            "chunk_size": len(chunk_text),
                            "entity_count": len([e for e in entities if text_pos <= e.start_char < text_end])
                        }
                    ))
        ...
        logger.warning(f"Error in entity-aware splitting: {e}, falling back to recursive splitting")
        return split_recursive(text, chunk_size=chunk_size, **kwargs)
...
def split_relation_aware(
    ...
    **kwargs
    ...
    try:
                ...
                if preserve_triples and is_in_triple:
                    ...
                    chunks.append(Chunk(
                        ...
                        metadata={
                            ...
                            "chunk_size": len(chunk_text),
                            "relation_count": len([r for r in relations if text_pos <= r.subject.start_char < text_end])
                        }
                    ))
        ...
        if current_chunk.strip():
            ...
            chunks.append(Chunk(
                ...
                metadata={
                    "method": "relation_aware",
                    "chunk_size": len(chunk_text),
                    "relation_count": len([r for r in relations if text_pos <= r.subject_start < text_end])
                }
            ))
        ...
        logger.warning(f"Error in relation-aware splitting: {e}, falling back to recursive splitting")
        return split_recursive(text, chunk_size=chunk_size, **kwargs)
...
def split_graph_based(
    ...
    **kwargs
    ...
    if not SEMANTIC_EXTRACT_AVAILABLE or not NETWORKX_AVAILABLE:
        logger.warning("Required dependencies not available, falling back to recursive splitting")
        return split_recursive(text, chunk_size=chunk_size, **kwargs)
    
    try:
        # Extract entities and relations to build graph
        ner_extractor = NERExtractor(method=kwargs.get("ner_method", "ml"), **kwargs)
        relation_extractor = RelationExtractor(method=kwargs.get("relation_method", "ml"), **kwargs)
        ...
        if strategy == "community":
            ...
            return chunks if chunks else split_recursive(text, chunk_size=chunk_size, **kwargs)
        ...
        logger.warning(f"Error in graph-based splitting: {e}, falling back to recursive splitting")
        return split_recursive(text, chunk_size=chunk_size, **kwargs)
...
def split_ontology_aware(
    ...
    **kwargs
    ...
    return split_entity_aware(
        ...
        preserve_entities=preserve_concepts,
        **kwargs
    )
...
def split_hierarchical(
    ...
    **kwargs
    ...
    if "paragraph" in levels:
        return split_by_paragraphs(text, chunk_size=chunk_sizes[0] if chunk_sizes else 1000, **kwargs)
    
    # Fall back to sentence level
    return split_by_sentences(text, chunk_size=chunk_sizes[0] if chunk_sizes else 1000, **kwargs)
...
def split_topic_based(
    ...
    **kwargs
    ...
    logger.info(f"Topic-based splitting using semantic transformer (full {model} implementation pending)")
